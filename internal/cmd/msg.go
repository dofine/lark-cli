...
Examples:
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if msgType == "text" && (strings.Contains(msgSendText, "@{") || strings.Contains(msgSendText, "**") || strings.Contains(msgSendText, "[")) {
			msgType = "post"
			fmt.Fprintf(os.Stderr, "note: auto-upgraded --msg-type from text to post (mentions/markdown/link detected)\n")
		}
...
Examples:
  ...
  lark msg edit --message-id om_xxx --text "**Bold** and @{ou_xxx} mention"`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if msgType == "text" && (strings.Contains(msgEditText, "@{") || strings.Contains(msgEditText, "**") || strings.Contains(msgEditText, "[")) {
			msgType = "post"
			fmt.Fprintf(os.Stderr, "note: auto-upgraded --msg-type from text to post (mentions/markdown/link detected)\n")
		}
